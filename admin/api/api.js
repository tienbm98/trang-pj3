...
const loaispDb = require("../models/loaisanpham.model")
...
router.get('/category', async (req, res) => {
  let prs = await loaispDb.find();
  res.json(prs);
});

router.get('/category/:id', async (req, res) => {
  let prs = await loaispDb.findOne({ Name: req.params.id });;
  res.json(prs);
});

router.get('/product/category/:id', async (req, res) => {
  let id = req.params.id;
  let product = await prDb.find({ Category: id });
  return res.json(product);
})
